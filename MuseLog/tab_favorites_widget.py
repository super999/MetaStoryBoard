...
class TabFavoritesWidget(QWidget):
	...
	def __init__(self, parent: Optional[QWidget] = None) -> None:
		...
		self._favorites_file = self._init_config_path()
	...
	def _init_config_path(self) -> str:
		...
		try:
			...
			logging.debug("无法创建配置目录: %s", config_dir, exc_info=True)
		return os.path.join(config_dir, "tab_favorites.json")

	def _load_favorites(self) -> None:
		try:
			with open(self._favorites_file, "r", encoding="utf-8") as handle:
				data = json.load(handle)
		except FileNotFoundError:
			data = {}
		except Exception as exc:
			logging.exception("读取收藏夹配置失败: %s", self._favorites_file)
		...
		for entry in folders:
			...
			if not os.path.isdir(path):
				logging.info("收藏夹路径不存在, 自动跳过: %s", path)
				continue
			alias = entry.get("alias") or self._derive_alias(path)
			result.append({"path": path, "alias": alias})
		self._favorites = sorted(result, key=lambda item: item["alias"].lower())

	def _save_favorites(self) -> None:
		try:
			os.makedirs(os.path.dirname(self._favorites_file), exist_ok=True)
			with open(self._favorites_file, "w", encoding="utf-8") as handle:
				json.dump({"folders": self._favorites}, handle, ensure_ascii=False, indent=2)
		except Exception as exc:
			logging.exception("保存收藏夹失败: %s", self._favorites_file)
			QMessageBox.warning(self, "保存失败", f"写入收藏夹配置时发生错误：\n{exc}")
	...
	def on_add_folder(self) -> None:
		...
		for entry in self._favorites:
			if self._is_same_path(entry["path"], folder):
				QMessageBox.information(self, "已存在", "该文件夹已在收藏列表中。")
				return

		alias = self._ensure_unique_alias(self._derive_alias(folder))
		self._favorites.append({"path": folder, "alias": alias})
		self._favorites.sort(key=lambda item: item["alias"].lower())
		self._current_folder = folder
		self._save_favorites()
		self._refresh_tree()
		self._load_folder_entries(folder)
	...
	def _ensure_unique_alias(self, alias: str) -> str:
		existing = {entry["alias"].lower() for entry in self._favorites}
		base = alias
		counter = 2
		while alias.lower() in existing:
	...
	def _remove_favorite(self, path: str, *, silent: bool = False) -> None:
		...
		self._favorites = [entry for entry in self._favorites if not self._is_same_path(entry["path"], normalized)]
		if len(self._favorites) == before:
			return

		if self._current_folder and self._is_same_path(self._current_folder, normalized):
			self._current_folder = None
			self._current_entries.clear()
			self._apply_filter_to_list()
	...
	def _derive_alias(self, path: str) -> str:
